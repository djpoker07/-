...
# Sayfa Yapılandırması (Streamlit'in en başında olmalıdır)
st.set_page_config(
    page_title="PLANLAMA - Arama Uygulaması", page_icon="📍", layout="centered"
)
...
# Şifre doğru değilse uygulamanın devamını durdur
if not check_password():
  st.stop()

# --- ASIL UYGULAMA KODLARI ---
...
# ARAYÜZ
st.markdown(
    "<h2 style='text-align: center;'>MAHALLE ve MADDE ARAMA</h2>",
...
# Arama kutusunu sıfırlamak için session_state yönetimi
...
# Arama çubuğu ve Temizle butonunu yan yana yerleştirme
...
if "Mahalle" in mode:
  ...
  for _, row in df_mahalle.iterrows():
    ...
    val_kolluk = (
        "" if len(row) <= 2 or pd.isna(row.iloc[2]) else str(row.iloc[2])
    )
